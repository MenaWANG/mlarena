...
|:---|:---|:---|:---|:---|:---|
| **Cook's Distance (D)** | Overall influence of an observation on the model's predictions. | Quantifies how much all predictions would change if the observation were removed. Combines residual and leverage. | Larger D indicates more influence. Rules of thumb: $D > 4/n$ or $D > 1$. | Primarily Linear Regression | A single value per observation, reflecting its global impact. |
...
* **How it Works:**
    ...
    2.  For each data point $i$ in your dataset:
        ...
        d.  Calculate a metric of divergence between $\hat{Y}_{\text{full}}$ and $\hat{Y}_{(-i)}$. The most common and direct approach, inspired by Cook's Distance, is the mean squared differences of predictions:
            $$\text{Influence}_i = \frac{1}{n} \sum_{j=1}^{n} (\hat{y}_{j, \text{full}} - \hat{y}_{j, (-i)})^2$$            
