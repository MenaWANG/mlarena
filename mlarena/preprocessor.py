...
class PreProcessor(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X, y=None):
        ...
        if self.target_encode_cols:
            ...
            self.target_transformer = Pipeline(steps=[
                ...
                ('target_encoder', TargetEncoder(smooth=self.target_encode_smooth, cv=5))
            ])
    ...
    def transform(self, X):
        ...
        if self.target_encode_cols and hasattr(self, 'target_transformer'):
            target_encoded_data = self.target_transformer.transform(X[self.target_encode_cols])
            target_encoded_df = pd.DataFrame(target_encoded_data, columns=self.target_encode_cols, index=X.index)
        ...
        for col in categorical_cols:
            ...
            if n_unique > high_cardinality_threshold:
                ...
                if rare_categories > 0:
                    ...
                    analysis['encoding_note'] = "Recommend target encoding due to high cardinality and no rare categories"
                    print(analysis['encoding_note'])
            elif n_unique <= 2:
                onehot_encode_cols.append(col)
                analysis['encoding_note'] = "Recommend one-hot encoding as the number of unique value is less or equal to 2"
                print(analysis['encoding_note'])
                ...
                if rare_categories > 0:
                    ...
                    print(f"""Recommend target encoding with higher smoothing due to moderate cardinality and rare category""")
                elif prefer_target:
                    target_encode_cols.append(col)
                    analysis['encoding_note'] = """Either encoding will do due to moderate cardinality and sufficient sample size, feature added to target_encode_cols based on user preference"""
                    print(analysis['encoding_note'])
                else:
                    onehot_encode_cols.append(col)
                    analysis['encoding_note'] = """Either encoding will do due to moderate cardinality and sufficient sample size, feature added to onehot_encode_cols based on user preference"""
                    print(analysis['encoding_note'])
    ...
    def filter_feature_selection(X: pd.DataFrame, 
        ...
        analysis = analysis.sort_values('mutual_info_normalized', ascending=False).drop(['is_discrete'], axis=1).reset_index(drop=True)
