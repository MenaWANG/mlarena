...
def test_preprocessor_fit_transform():
    ...
    assert X_transformed.shape[1] >= X.shape[1]  # Should be larger due to one-hot encoding
...
def test_preprocessor_transform():
    ...
    X_train = pd.DataFrame(X_train, columns=['feature1', 'feature2', 'feature3', 'feature4', 'feature5'])
    X_train['cat_feature'] = np.random.choice(['A', 'B', 'C'], size=100)
    
    # Create test data
    print("Generating test data...")
    X_test = pd.DataFrame(np.random.randn(50, 5), columns=['feature1', 'feature2', 'feature3', 'feature4', 'feature5'])
    ...
    assert X_test_transformed.shape[1] >= X_test.shape[1]  # Should be larger due to one-hot encoding
